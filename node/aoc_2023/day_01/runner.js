...
7pqrstsixteen`;
  ...
  testSum = getTotal(testInput);
  if (testSum === 281) {
    console.log("test sum is good!");
  } else {
    console.error("test sum is bad!");
  }

  console.log(`result of puzzle: ${getTotal(puzzleInput)}`);
}
...
function getLineVal(line) {
  // let found = Array.from(line.matchAll(g));
  ...
  // console.log(`found in regex: ${found}`);
  found = found.map((val) => {
  ...
  // console.log(`mapped from regex: ${found}`);
  // console.log(`adding ${found[0] * 10} to ${found[found.length-1]}`);
  
  val = found[0] * 10 + found[found.length-1];
